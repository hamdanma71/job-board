...
- **الذكاء الاصطناعي:** OpenAI (`gpt-4o-mini`، JSON mode) مع بدائل آمنة عند غياب المفتاح؛ **تحليل السيرة (`parseCV`) يستخدم مُحلِّلًا استدلاليًّا محليًّا** عند غياب المفتاح يستخرج المهارات/سنوات الخبرة/النبذة من نصّ السيرة الفعليّ (لا بيانات ثابتة).
