...
export const metadata = {
  title: "حلول التوظيف للشركات | JobMatch",
  description: "انشر الوظائف، ابحث في قاعدة المرشّحين، وأدِر التوظيف بنظام ATS وأدوات ذكاء اصطناعي.",
};
  ...
  return (
    <main>
      ...
      <section style={{ padding: "3rem 1.5rem", backgroundColor: "var(--surface)", borderBottom: "1px solid var(--border-light)" }}>
        <div className="container" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "2rem", textAlign: "center" }}>
          <div>
            ...
            <div className="text-muted" style={{ fontSize: "1.1rem" }}>{t("employersPage.statActiveResumes")}</div>
          </div>
          <div>
            <div style={{ fontSize: "3rem", fontWeight: "bold", color: "var(--primary)" }}>95%</div>
            <div className="text-muted" style={{ fontSize: "1.1rem" }}>{t("employersPage.statMatchAccuracy")}</div>
          </div>
          <div>
            <div style={{ fontSize: "3rem", fontWeight: "bold", color: "var(--primary)" }}>-80%</div>
            <div className="text-muted" style={{ fontSize: "1.1rem" }}>{t("employersPage.statTimeReduction")}</div>
          </div>
        </div>
      </section>
      ...
      <section className="container" style={{ padding: "5rem 1.5rem" }}>
        <h2 style={{ textAlign: "center", fontSize: "2.5rem", fontWeight: "bold", marginBottom: "3rem" }}>{t("employersPage.featuresTitle")}</h2>
        
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "2rem" }}>
          
          <div className="card hover-scale" style={{ textAlign: "center", padding: "2rem" }}>
            <div style={{ fontSize: "3rem", marginBottom: "1rem" }}>🧠</div>
            <h3 style={{ fontSize: "1.25rem", fontWeight: "bold", marginBottom: "1rem" }}>{t("employersPage.feature1Title")}</h3>
            <p className="text-muted">{t("employersPage.feature1Desc")}</p>
          </div>

          <div className="card hover-scale" style={{ textAlign: "center", padding: "2rem" }}>
            <div style={{ fontSize: "3rem", marginBottom: "1rem" }}>📂</div>
            <h3 style={{ fontSize: "1.25rem", fontWeight: "bold", marginBottom: "1rem" }}>{t("employersPage.feature2Title")}</h3>
            <p className="text-muted">{t("employersPage.feature2Desc")}</p>
          </div>
